...
import owlstack
from owlstack import DEFAULT_BASE_URL, Owlstack
...
def test_the_api_key_is_required(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("OWLSTACK_API_KEY", raising=False)
...
def test_the_api_key_falls_back_to_the_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OWLSTACK_API_KEY", "osk_from_env")
...
def test_the_default_base_url_includes_the_api_version() -> None:
    assert DEFAULT_BASE_URL == "https://api.owlstack.app/api/v1"

    with Owlstack(api_key=API_KEY) as client:
        assert client.base_url == DEFAULT_BASE_URL


def test_a_trailing_slash_on_the_base_url_does_not_double_up() -> None:
    with Owlstack(api_key=API_KEY, base_url=f"{BASE_URL}/") as client:
...
def test_owlstack_is_exported_under_the_typescript_spelling_too() -> None:
    assert owlstack.OwlStack is owlstack.Owlstack


@respx.mock
def test_every_request_carries_the_api_key_and_user_agent(client: Owlstack) -> None:
    ...
    assert headers["user-agent"] == "owlstack-python"
    assert headers["accept"] == "application/json"


@respx.mock
def test_request_reaches_an_endpoint_the_sdk_does_not_wrap(client: Owlstack) -> None:
...
def test_max_retries_must_be_at_least_one() -> None:
    with pytest.raises(ValueError, match="max_retries"):
        Owlstack(api_key=API_KEY, max_retries=0)
...
def test_an_injected_http_client_is_used_and_left_open() -> None:
    ...
    client = Owlstack(api_key=API_KEY, base_url=BASE_URL, http_client=transport_client)
